...
SIZE_T FMallocScaledImpl::TMemPoolTable::GetPoolIndex(SIZE_T BlockSize, SIZE_T BaseIndex, SIZE_T MinBaseIndex, SIZE_T SubIndexCountShift)
{
	//SIZE_T BlockIndex = MinBaseIndex + BaseIndex;
	//SIZE_T UpperPoolBlockSize = FMallocScaledMath::Pow2_64(BlockIndex);
	//SIZE_T LowerPoolBlockSize = BlockIndex == MinBaseIndex ? 0 : FMallocScaledMath::Pow2_64(BlockIndex - 1);
	...
	SIZE_T LowerPoolBlockSize = BlockIndex == MinBaseIndex ? 0 : ((SIZE_T)1 << (BlockIndex - 1));

	SIZE_T SpacingShift = BlockIndex == MinBaseIndex ? 5 : (BlockIndex - 1) - SubIndexCountShift;//(LowerPoolBlockSize) >> SubIndexCountShift;
	SIZE_T PoolIndex = ((BlockSize - LowerPoolBlockSize) >> SpacingShift);

	if (PoolIndex == ((SIZE_T)1 << SubIndexCountShift))
	{
		--PoolIndex;
	}
...
void FMallocScaledImpl::TMemPool::Init(SIZE_T InBaseIndex, SIZE_T InPoolIndex, SIZE_T InBlockSize, SIZE_T InPoolBlockSize)
{
	...
	this->PoolVMBlockSize = Align(InPoolBlockSize + MemPoolHdrSize, FMallocScaledVMBlock::GetPageSize());
...
{
	FMallocScaledVMBlock NewPoolVMBlock;

	if (BlockSize > PoolBlockSize)
	{
		PoolVMBlockSize = Align(MemPoolHdrSize + MemBlockHdrSize + MemBlockHdrOffsetSize + BlockSize, FMallocScaledVMBlock::GetPageSize());
...
void FMallocScaledImpl::TMemPool::DeletePool(TMemPoolHdr* Pool)
{
	...
	FMallocScaledVMBlock PoolVMBlock = std::move(Pool->PoolVMBlock);
	PoolVMBlock.Free();
}
...
FMallocScaledImpl::TMemPoolHdr* FMallocScaledImpl::TMemPool::FindNewHeadPool()
{
	auto PoolNode = PoolList.GetFirst();
...
void FMallocScaledImpl::TMemPool::Release()
{
...
bool FMallocScaledImpl::TMemPoolTableEntry::Init(SIZE_T InBaseIndex, SIZE_T InMinBaseBlockSize,
	SIZE_T InMaxBaseBlockSize, SIZE_T InPoolBlockSize, SIZE_T InSubIndexCountShift)
{
	...
	if (InSubIndexCountShift > MaxSubIndexCountShift)
	{
		// log error;
		return false;
	}

	for (SIZE_T i = 0; i < ((SIZE_T)1 << InSubIndexCountShift); ++i)
	{
		SIZE_T BlockSize = TMemPoolTable::CalculatePoolBlockSize(InBaseIndex, i, InMinBaseBlockSize, InMaxBaseBlockSize, ((SIZE_T)1 << InSubIndexCountShift));
		Pools[i].Init(InBaseIndex, i, BlockSize, InPoolBlockSize);
	}

	this->SubIndexCountShift = InSubIndexCountShift;
	Initialized = true;
	return true;
}

void FMallocScaledImpl::TMemPoolTableEntry::Release()
{
	for (SIZE_T i = 0; i < ((SIZE_T)1 << SubIndexCountShift); ++i)
	{
		Pools[i].Release();
	}
...
{
	return ((SIZE_T)1 << SubIndexCountShift);
}
...
{
	bool Ok = false;
	SIZE_T DataBlockSize = Align(EntrySize * InEntryCount, FMallocScaledVMBlock::GetPageSize());
...
SIZE_T FMallocScaledImpl::TMemPoolTable::GetSubIndexCountShift()
{
...
bool FMallocScaledImpl::TMemPoolTable::Init(SIZE_T InMinBaseBlockSize, SIZE_T InMaxBaseBlockSize,
	SIZE_T InPoolBlockSize, SIZE_T InSubIndexCountShift)
{
#if	MALLOC_SCALED_MALLOC_DEBUG
	...
	if (Ok)
	{
		...
		{
			BaseEntries[i].Init(i, InMinBaseBlockSize, InMaxBaseBlockSize, InPoolBlockSize, InSubIndexCountShift);
		...
		this->SubIndexCountShift = InSubIndexCountShift;
		this->SubIndexCount      = (SIZE_T)1 << InSubIndexCountShift;
...
void FMallocScaledImpl::TMemPoolTable::Release()
{
...
void* FMallocScaledImpl::MallocInternal(SIZE_T Size, uint32 Alignment)
{
	...
	if (Size &&
		...
		Alignment <= FMallocScaledVMBlock::GetPageSize())
	{
		...
		if (Ok)
		{
			SIZE_T PoolIdx = TMemPoolTable::GetPoolIndex(AdjustedBlockSize, BaseIndex, PoolTable.GetMinBaseIndex(), PoolTable.GetSubIndexCountShift());
...
void* FMallocScaledImpl::ReallocInternal(void* Addr, SIZE_T NewSize, uint32 NewAlignment)
{
	...
	if (NewSize &&
		...
		NewAlignment <= FMallocScaledVMBlock::GetPageSize())
	{
		void* NewPtr = nullptr;
	...
	if (!Initialized)
	{
		...
		if (Ok)
		{
			Ok = PoolTable.Init(MALLOC_SCALED_MIN_BASE_BLOCK_SIZE,
				...
				FMallocScaledMath::Log2_64(MALLOC_SCALED_SUBINDEX_COUNT));
...
void FMallocScaledImpl::Shutdown()
{
#if MALLOC_SCALED_MALLOC_DEBUG
...
{
	return FMallocScaledVMBlock::GetPageSize();
}
